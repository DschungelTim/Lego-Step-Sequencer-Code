...
# Midi Port initialisieren.
midiOutput = mido.open_output("LoopBe Internal MIDI 1")

# Definieren der Farbwerte der Legosteine, durch Testen ermittelt
# Wertebereich Hue: 0-180, Sat: 0-255
# Datenstruktur: [Hue_min, Hue_max, Sat_min, Sat_max]
gruen = [60, 90, 153, 255]
...
# Übersetzungstabelle für das Senden der Felder über Midi.
felderDict = {
    (1, 1) : 0,
    (2, 1) : 1,
    (3, 1) : 2,
...
# Funktion, um die Schwerpunkte der einzelnen, erkannten Steine zu berechnen.
...
# Funktion, um die erkannten X- und Y-Koordinaten der ermittelten Schwerpunkte der Steine in unser definiertes Raster einzuordnen.
def PinKoord (SPunktarray) :
    dx = x1 - x0    # Länge unserer X-Achse
    dy = y1 - y0    # Länge unserer Y-Achse
    yxFelder = [[], []]
    Wert = 0
    for i in range(0, len(SPunktarray)) :   # X- und Y-Koordinate einlesen.
        X = SPunktarray [i] [0]
        Y = SPunktarray [i] [1]
        for i in range (0, 10) :
            if (x0 + (i/10) * dx) < X <= (x1 - ((9-i)/10) * dx) :               # Schauen, in welchem Feld auf der X-Achse der Stein liegt.   
                if i == 8 or i == 9:                                            # Schauen, ob der erkannte Stein im Bereich der Slider liegt.
                    Wert = ((Y-127)/192)*127
                    Wert = round(Wert)   
                    sendMidiNote(i+24, 127-Wert)                                # Senden der Position des Steines, um Sliderwerte zu aktualisieren.
                if 0 <= i <= 7:                                                 
                    xFelder = yxFelder[1]
                    xFelder.append(i+1)   
                    for i in range (0, 4) :
                        if (y0 + (i/4) * dy) < Y <= (y1 -((3 - i)/4) * dy) :    # Schauen, in welchem Feld auf der Y-Achse der Stein liegt.
                            yFelder = yxFelder[0]
                            yFelder.append(i+1)
    return yxFelder
...
# Über die Übersetzungstabelle werden die hier errechneten Werte in die vom Javascript erwarteten umgerechnet.
...
# Counter, um nur bei dem allerersten Frame die Ecksteine zu erkennen und damit das Koordinatensystem zu initialisieren.
counter = 0
...
while cap.isOpened():
    ...
    # Eckpunkte des Koordinatensystems erkennen und festlegen.
    try:
        if counter == 0:
        ...
        # Funktionen aufrufen, um Schwerpunkte zu berechen, Ergebnis wird als Array gespeichert mit jeweils x- und y-Koordinaten.
        ...
        # Kurzes Delay, damit der Midiport nicht überlastet wird.
        time.sleep(0.1)
    except:
        pass
